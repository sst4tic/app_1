...
class OperationCreatingRepo implements AbstractCreatingOperation {
  ...
  @override
  Future<List<ChildData>> getOperationTypes() async {
  ...
  Future submitMoving(
      {required billsIdTo,
      required billsIdFrom,
      required total,
      comments}) async {
    var url =
        '${Constants.API_URL_DOMAIN}action=operations_moving&bills_id_from=$billsIdFrom&bills_id_to=$billsIdTo&total=$total&comments=$comments';
    final response =
        await http.get(Uri.parse(url), headers: Constants.headers());
    final body = jsonDecode(response.body);
    return body;
  ...
  Future submitAddition(
      ...
      comments}) async {
    var url =
        '${Constants.API_URL_DOMAIN}action=operations_addition&bills_id=$billsId&type=$type&total=$total&article_id=$articleId&invoice_id=$invoiceId&comments=$comments';
    final response =
        await http.get(Uri.parse(url), headers: Constants.headers());
    final body = jsonDecode(response.body);
    return body;
